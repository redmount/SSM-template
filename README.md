...
- [3. 实体创建规则](#3-实体创建规则)
    ...
    - [3.3 复杂实体](#33-复杂实体)
    - [3.4 自定义实体](#34-自定义实体)
- [4. 抽象Controller](#4-抽象controller)
    - [4.1. 提供列表的分页/条件/关键字查询](#41-提供列表的分页条件关键字查询)
    - [4.2. 根据pk查询单条实体](#42-根据pk查询单条实体)
    - [4.3. 保存/修改实体](#43-保存修改实体)
    - [4.4. 删除实体](#44-删除实体)
- [5. 最佳实践](#5-最佳实践)
    - [5.1. 准备阶段](#51-准备阶段)
    - [5.2. 生成阶段](#52-生成阶段)
...
```特殊情况```是指需要在发生异常的情况下进行进一步操作的时候,需要进行catch拦截.
...
## 9.3. MyBatis二级缓存的开启
在 ```test/java/ModelCommentGenerator.java```文件中,构造函数```public ModelCommentGenerator()```有几个开关,其中```this.cacheMapper = false;```为默认值,将其改为```true```就会在生成Mapper时,增加@CacheNemespace注解,从而开启MyBatis自身的二级缓存.

> 开启的二级缓存机制是以单表作为一个Namespace单位,当单表没有通过MyBatis更新/插入等导致表数据变化的操作时,会自动缓存查询出的数据.一旦单表有了数据变化,以往的缓存数据自动作废.
>
> 基于以上机制,建议将数据不经常变化的表进行开启二级缓存以提高性能,经常变化的业务表不开启二级缓存,日后会提供针对单表的二级缓存开关.
>
> 在开发阶段,可能会任务对数据进行修改,定制测试数据和观察数据的变化,此时如果开启了MyBatis自身的二级缓存,会使手动修改的数据不生效,因为没有通过MyBatis进行数据的修改,MyBatis会自动从缓存中读取数据.
>
> 所以,建议在开发阶段不开启二级缓存.
...
2. 无限级的数据关联查询.
3. Condition扩展到子实体的条件,例如:查询出所有班主任姓李的班级.
4. SQL优化,采用拼装SQL的方式进行查询,而不是循环查询.
5. 用户体系
